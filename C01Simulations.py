...
def create_order_api(date, team, game_id):
    ...
    if order_api_df['batting_order'].sum() != 45 and order_api_df['batting_order'].sum() != 0:
        print(f"{team} MLB API batting orders are present but do not add up to 45.")
...
def fill_missing_batting_order(df):
    ...
    missing_rows = missing_rows.sort_values(by=['pa_b_r', 'b1_b_r'], ascending=False)
...
def create_batter_objects(batter_df, order_df, scale_batter_stats, scale_batter_stats_steamer, impute_batter_stats):
    # Merge on id with MLB API lineups
...
def create_pitcher_objects(pitcher_df, scale_pitcher_stats, scale_pitcher_stats_steamer, impute_pitcher_stats):
    ...
    pitcher_df['relief_IP'].fillna(0, inplace=True)
...
def determine_pull(game, models):
    if game.top_bot == "Top":        
    ...
    inputs = np.array([
        ...
        game.pitching.faced, game.pitching.reached, game.pitching.OUT,
        *inning_dummies,
        *out_dummies,
    ...
    pull_prob = models['predict_pulls'].predict_proba(inputs)[0, 1]
...
def determine_pitcher(game, opener_list, models):
    def choose_pitcher(team_pitchers, starter, starter_pulled_attr, team_score, opp_score, opener_list):
        ...
        if not starter_pulled:
            ...
            setattr(game, starter_pulled_attr, determine_pull(game, models))
        ...
        if getattr(game, starter_pulled_attr):
            ...
            predictions_proba = models['predict_leverage'].predict_proba(leverage_inputs)
            leverage_df = pd.DataFrame(predictions_proba, columns=models['predict_leverage'].classes_)
...
def determine_event(game, park_object, models, debug=False):
    ### Batter Attributes
    ...
    # Choose Event
    event = random.choices(events_list, weights=np.array(models['predict_all'].predict_proba(np.array(inputs_pa, dtype=np.float32).reshape(1, -1))).ravel(), k=1)[0]

    pa_summary = list(inputs_pa) + [event]
...
def determine_error(game, event, models):
    ...
    error_prob = models['predict_errors'].predict_proba(inputs)[0, 1]
...
def determine_out_locations(game, event, error, double_play, models, debug):
    def get_out_prob(base_flag):
        if base_flag is not None:
            input_list = [int(event == e) for e in events_list]
            base_dummies = [
            ...
            return models['predict_out_bases'].predict_proba(model_inputs).tolist()[0][1]
        return 0
...
def determine_event_results(game, startInt, event, out_ab, out_1b, out_2b, out_3b, blocked_1b, blocked_2b, blocked_3b, error, double_play, models):
    event_dummies = [int(event == e) for e in events_list]
    ...
    probs = models['predict_advances'].predict_proba(inputs)[0]
...
def sim_ab(game, opener_list, park_object, models, debug=False, log=False):
    if game.top_bot == "Top":
        if game.home_starter_pulled == False:
            game = determine_pitcher(game, opener_list, models)
    ...
    event, pa_summary = determine_event(game, park_object, models, debug)
    ...
    error = determine_error(game, event, models)
    ...
    double_play = determine_dp(game, event, models)
    if debug == True and double_play == True:
        print("There's a double play!")
    
    out_ab, out_1b, out_2b, out_3b = determine_out_locations(game, event, error, double_play, models, debug)
    if debug == True:
    ...
    if (game.outs + outs_on_play < 3) or (event in ['b1', 'b2', 'b3', 'hr', 'bb', 'hbp']):
        ...
        if out_3b == 1:
            ...
            blocked_2b = 0
            blocked_3b = 0
            base_3b = determine_event_results(game, 3, event, out_ab, out_1b, out_2b, out_3b, blocked_1b, blocked_2b, blocked_3b, int(error), int(double_play), models)
        else:
            base_3b = "N/A"
        ...
        if out_2b == 1:
            ...
            base_2b = determine_event_results(game, 2, event, out_ab, out_1b, out_2b, out_3b, blocked_1b, blocked_2b, blocked_3b, int(error), int(double_play), models)
        else:
            base_2b = "N/A"
        ...
        if out_1b == 1:
            ...
            base_1b = determine_event_results(game, 1, event, out_ab, out_1b, out_2b, out_3b, blocked_1b, blocked_2b, blocked_3b, int(error), int(double_play), models)
        else:
            base_1b = "N/A"
        ...
        if out_ab == 1:
            ...
            base_ab = determine_event_results(game, 0, event, out_ab, out_1b, out_2b, out_3b, blocked_1b, blocked_2b, blocked_3b, int(error), int(double_play), models)
        else:
            base_ab = "N/A"
        ...
        if game.on_3b is not None:
            if base_3b == "to_score":
                game.on_3b.R += 1
                game.on_3b.pitcher.ER += (1 * (1-int(error)) * game.on_3b.charged)
                game.on_3b.pitcher.ER_inning += (1 * (1-int(error)) * game.on_3b.charged)
...
def sim_inning(game, opener_list, park_object, models, innings, debug=False, log=False):
    game.outs = 0
    game.on_1b = None
    game.on_2b = None
    game.on_3b = None
    ...
    while game.outs < 3:        
        ...
        if game.on_3b is None and game.on_2b is not None:
            ...
            sba_3b_rate = models['predict_sba_3b'].predict_proba(model_inputs).tolist()[0][1]
            if debug == True:
                print(f"3B Attempt Rate: {sba_3b_rate}")
            
            if sba_3b_roll < sba_3b_rate:
                sb_3b_roll = random.random()
                sb_3b_rate = models['predict_sb_3b'].predict_proba(model_inputs).tolist()[0][1]
                if debug == True:
        ...
        if game.on_2b is None and game.on_1b is not None:
            ...
            sba_2b_rate = models['predict_sba_2b'].predict_proba(model_inputs).tolist()[0][1]
            if debug == True:
                print(f"2B Attempt Rate: {sba_2b_rate}")         
            
            if sba_2b_roll < sba_2b_rate:
                sb_2b_roll = random.random()
                sb_2b_rate = models['predict_sb_2b'].predict_proba(model_inputs).tolist()[0][1]
                if debug == True:
        ...
        game = sim_ab(game, opener_list, park_object, models, debug, log)
...
def sim_game(game_template, opener_list, park_object, models, innings=9, debug=False, log=False):
    ...
    while game.inning <= game.innings:
        game = sim_inning(game, opener_list, park_object, models, innings, debug, log)
...
def sim_game_batch(game_template, opener_list, park_object, models, innings=9, debug=False, log=False, batch_size=50):
    return [
        sim_game(game_template, opener_list, park_object, models,
                 innings=innings, debug=debug, log=log)
...
# %%
# Log plate appearance summary to CSV
...
def display_results(away_team, away_lineup_symbol, away_starter, home_team, home_lineup_symbol, home_starter, game_num, odds_df, game_scores_df, display_gambling_info):
    GREEN = "\033[92m"
    RED = "\033[91m"
    RESET = "\033[0m"
    ...
    if display_gambling_info:
        try:
            ...
            away_label = f"{away_team}{away_lineup_symbol}"
            home_label = f"{home_team}{home_lineup_symbol}"
    
            away_line = (
                f"{away_label:<5} "
                f"{away_mean:>5.2f} ({away_win_pct:>5.2f}%) | "
                f"ML {colorize_return(r['ML1']['return'])} ({fmt_odds(r['ML1']['odds'])}) | "
                f"SPR {fmt_line(spread)} {colorize_return(r['Spread1']['return'])} ({fmt_odds(r['Spread1']['odds'])}) | "
                f"OVR {fmt_line(ou)} {colorize_return(r['Ou1']['return'])} ({fmt_odds(r['Ou1']['odds'])}) | "
                f"SP {away_starter}"
            )
            
            home_line = (
                f"{home_label:<5} "
                f"{home_mean:>5.2f} ({home_win_pct:>5.2f}%) | "
                f"ML {colorize_return(r['ML2']['return'])} ({fmt_odds(r['ML2']['odds'])}) | "
                f"SPR {fmt_line(-spread)} {colorize_return(r['Spread2']['return'])} ({fmt_odds(r['Spread2']['odds'])}) | "
                f"UND {fmt_line(ou)} {colorize_return(r['Ou2']['return'])} ({fmt_odds(r['Ou2']['odds'])}) | "
                f"SP {home_starter}"
            )
        except:
            print("Can't display gambling information")
            away_line = f"{away_team:<4} {away_mean:>5.2f} ({away_win_pct:>5.2f}%)"
            home_line = f"{home_team:<4} {home_mean:>5.2f} ({home_win_pct:>5.2f}%)"
    else:
        away_line = f"{away_team:<4} {away_mean:>5.2f} ({away_win_pct:>5.2f}%)"
        home_line = f"{home_team:<4} {home_mean:>5.2f} ({home_win_pct:>5.2f}%)"
...
def expected_returns(home_team, game_num, odds_df, game_scores_df):
    ...
    if home_games.empty or game_num > len(home_games):
        print(f"No odds found for {home_team} game #{game_num}")
        return
